	...
	<thead> 
      ...
      <th class="price">{l s='Price' mod='envoimoinscher'}
        <div class="margin-form add-tooltip" title="
             {l s='Real price: the price automatically calculated on boxtal.com.' mod='envoimoinscher'}<br />
		...
		{if isset($carriersByType) && $carriersByType && sizeof($carriersByType)}
            ...
            {foreach from=$carriersByType key=type item=list}
                {foreach from=$list key=o item=offer}
                    ...
                    <tr{if !in_array($offer.emc_operators_code_eo, $operators)} class="disabled"{/if}>
                        ...
                        <td class="offer">
                            ...
                            {if $offer.is_parcel_dropoff_point_es == 1 && in_array($offer.emc_operators_code_eo, $operators)}
                        ...
                        <td class="area">
                            ...
                            {if $offer.zone_es_es == 1}<span class="zone_es">{l s='ES' mod='envoimoinscher'}</span>{/if}
                        ...
                        <td class="status">
                            <div class="hide">
                                <input type="checkbox" name="offers[]" value="{$offer.offerCode|escape:'htmlall':'UTF-8'}" id="offer{$offer.id_es|escape:'htmlall':'UTF-8'}" {if $offer.id_carrier > 0 && in_array($offer.emc_operators_code_eo, $operators)} checked="checked"{/if}{if $disableServices} disabled="disabled"{/if} />
                            </div>
                            <img src="../img/admin/{if $offer.id_carrier > 0 && in_array($offer.emc_operators_code_eo, $operators)}enabled{else}disabled{/if}.gif" alt="{if $offer.id_carrier > 0 && in_array($offer.emc_operators_code_eo, $operators)}true{else}done{/if}" class="toggleCarrier" onClick="EMCtoggleCarrier($(this))" />
                        </td>
                        <td class="price">
                          {if in_array($offer.emc_operators_code_eo, $operators)}
                            <div id="price_switch_{$offer.id_es|escape:'htmlall':'UTF-8'}" class="EMC_switch switch prestashop-switch clear"{if $offer.id_carrier == '' || !in_array($offer.emc_operators_code_eo, $operators)} style="display:none;"{/if}>
