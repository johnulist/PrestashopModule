...
<form method="POST" action="{$EMC_link|escape:'htmlall':'UTF-8'}&EMC_tabs=settings">
  ...
  <fieldset>
		...
		<div class="margin-form">
			<select id="EMC_filter_type_order" name="EMC_filter_type_order[]" multiple size="3">
                {if (isset($EMC_config.EMC_FILTER_TYPE_ORDER))}
                    {assign var=selected_types value=";"|explode:$EMC_config.EMC_FILTER_TYPE_ORDER}
                {/if}
                <option value="0" {if !isset($selected_types) || (isset($selected_types) && "0"|in_array:$selected_types)}selected{/if}>{l s='EnvoiMoinsCher orders' mod='envoimoinscher'}</option>
                <option value="1" {if !isset($selected_types) || (isset($selected_types) && "1"|in_array:$selected_types)}selected{/if}>{l s='Non EnvoiMoinsCher orders' mod='envoimoinscher'}</option>
                <option value="2" {if !isset($selected_types) || (isset($selected_types) && "2"|in_array:$selected_types)}selected{/if}>{l s='Invalid or incomplete orders' mod='envoimoinscher'}</option>
			</select>
        </div>
		<div class="clear both"></div>
